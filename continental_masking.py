...
# ax=Basemap()
# for poly in region_polygons['Oceania']:
...
varoutdict[dataset]['TXx']=pdf.PDF_Processing('TXx')
varoutdict[dataset]['TXx'].mask_for_ref_period_data_coverage(input_data,ref_period,check_ref_period_only=False,target_periods=target_periods,landmask=landmask,required_coverage=0.8,dataset=dataset)

# mask=varoutdict[dataset]['TXx']._masks['global'].copy()
# mask[np.isfinite(mask)]=1
# varoutdict[dataset]['TXx'].plot_map(mask,
#                                     color_bar=False,
#                                     show=False)



varoutdict[dataset]['TXx'].derive_regional_masking(shift_lon=-180.0,region_polygons=continent_polygons,region_type='continental',dataset=dataset)
